...
const system=`You are the Reimagine Director inside AvoMiez Director V2.7. Your task is NOT merely to append instructions or rewrite a prompt. You must rethink an analyzed short-form video using its source mechanics, the creator's preserve/change locks, optional references, selected remix mode, duration, current editable idea, current H3 prompt and optional SECONDARY TONE OVERLAY. ${AVOMIEZ_LOCK}\n\nCreate a genuinely improved or transformed 8–15 second vertical AI-video concept. The tone overlay is additive and may be blended with any primary genre; never replace the primary story logic, reference roles or character identity. Preserve only what the creator explicitly locks. Prioritize first-second hook, visual clarity, retention, escalating readable action, strong payoff, replay/loop potential, realistic model-executable choreography and strict AvoMiez consistency. Never claim guaranteed virality. Avoid engagement bait, unsafe challenges, copyrighted character imitation and unnecessary text overlays. IMPORTANT: every final h3Prompt must stay within the current hard limit of ${VIDEO_PROMPT_MAX_CHARS} characters. Return valid JSON only.`;
...
export default async function handler(req,res){
  ...
  try{
    ...
    const content=[{type:'text',text:user({...b,toneOverlay:tone})}];
    imgs.forEach((url,i)=>{if(typeof url==='string'&&url.startsWith('data:image/'))content.push({type:'image_url',image_url:{url},detail:'low'})});
    const r=await callM3({messages:[{role:'system',content:system},{role:'user',content}],temperature:['surprise','different','inspire'].includes(b.mode)?.78:.52,maxTokens:3600});
    const data={...r.data};
    const fit=await enforceVideoPromptLimit(data.h3Prompt,{label:'Reimagine Studio video prompt'});
    data.h3Prompt=fit.prompt;
    return send(res,200,{...data,_meta:{model:r.model,usage:r.usage,referencesAnalyzed:imgs.length,toneOverlay:tone,promptLimit:VIDEO_PROMPT_MAX_CHARS,promptChars:fit.finalChars,promptOriginalChars:fit.originalChars,promptCompacted:fit.compacted,promptCompaction:fit.method}});
