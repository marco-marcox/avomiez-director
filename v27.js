(()=>{
  ...
  if(!window.AVOMIEZ_UI_VERSION)window.AVOMIEZ_UI_VERSION='V2.7';
  const version=()=>window.AVOMIEZ_UI_VERSION||'V2.7';
  ...
  function panel(scope,title,subtitle){
    ...
    wrap.innerHTML=`<div style="margin-top:12px;padding:10px;border:1px solid #58401f;border-radius:13px;background:linear-gradient(180deg,#171109,#0d1119)">
      <div class="analysisBadge" style="background:#241a0d;color:#ffd98a">${version()} TONE OVERLAY</div>
  ...
  function getTone(scope){return{name:$(`#v27-${scope}-tone`)?.value||'none',strength:$(`#v27-${scope}-strength`)?.value||'medium',expression:$(`#v27-${scope}-expression`)?.value||'balanced',note:$(`#v27-${scope}-note`)?.value||''}}
  function setTone(scope,t){if(!t)return;const tone=$(`#v27-${scope}-tone`),strength=$(`#v27-${scope}-strength`),expression=$(`#v27-${scope}-expression`),note=$(`#v27-${scope}-note`);if(tone&&LABELS[t.name])tone.value=t.name;if(strength&&['light','medium','strong'].includes(t.strength))strength.value=t.strength;if(expression&&['social-safe','balanced','cinematic-intense'].includes(t.expression))expression.value=t.expression;if(note)note.value=t.note||'';updateInfo(scope)}
  function updateInfo(scope){const t=getTone(scope),box=$(`#v27-${scope}-info`);if(!box)return;if(t.name==='none')box.textContent='No secondary overlay — primary genre stays unchanged.';else box.textContent=`${EMOJI[t.name]} ${LABELS[t.name]} · ${t.strength} · ${t.expression}${t.note.trim()?' · custom note active':''}`}
  function bindScope(scope){const box=$('#v27Tone-'+scope);if(!box||box.dataset.bound)return;box.dataset.bound='1';['tone','strength','expression','note'].forEach(k=>{const el=$(`#v27-${scope}-${k}`);if(el){el.addEventListener('input',()=>updateInfo(scope));el.addEventListener('change',()=>updateInfo(scope))}});updateInfo(scope)}
  function copyTone(from,to){setTone(to,getTone(from))}

  document.addEventListener('click',e=>{if(e.target.closest('#v25ContinueExtend'))setTimeout(()=>copyTone('create','extend'),0);if(e.target.closest('#useCont'))setTimeout(()=>copyTone('analyze','extend'),0)},true);

  const nativeFetch=window.fetch.bind(window);
  window.fetch=async function(input,init){
    try{
      const url=new URL(typeof input==='string'?input:input.url,location.href),method=String(init?.method||'GET').toUpperCase();
      if(method==='POST'&&typeof init?.body==='string'){
        ...
        if(scope){const body=JSON.parse(init.body),tone=getTone(scope);body.toneOverlay=tone;state.last[scope]={...tone};init={...init,body:JSON.stringify(body)}}
      }
    }catch{}
    return nativeFetch(input,init);
  };

  function toneBadge(scope,id,anchorSelector){const t=state.last[scope],anchor=$(anchorSelector);if(!t||!anchor)return;let b=$('#'+id);if(!b){b=document.createElement('div');b.id=id;b.style.cssText='margin:7px 0;padding:7px 10px;border:1px solid #58401f;border-radius:10px;background:#171109;color:#ffd98a;font-size:11px;font-weight:800';anchor.insertAdjacentElement('afterend',b)}b.textContent=t.name==='none'?'Tone overlay: None':`${EMOJI[t.name]} Tone: ${LABELS[t.name]} · ${t.strength} · ${t.expression}`}

  function refresh(){installPanels();const v=version();const h=$('.top h1');if(h&&h.textContent!==`AvoMiez Director ${v}`)h.textContent=`AvoMiez Director ${v}`;$$('.analysisBadge').forEach(b=>{if(b.textContent.includes('TONE OVERLAY'))b.textContent=v+' TONE OVERLAY'});toneBadge('create','v27CreateToneBadge','#v26CreateMeter');toneBadge('extend','v27ExtendToneBadge','#v26ExtendMeter');toneBadge('analyze','v27AnalyzeToneBadge','#v26AnalyzeMeter')}

  let timer;const obs=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(refresh,30)});obs.observe(document.body,{subtree:true,childList:true,characterData:true});refresh();
  console.log('AvoMiez Director universal tone overlay ready · '+version());
})();
